...
# [Class Setup info]---------------------------------------------------------------------------------------------------[+]
    ...
    logging.info('-                                                               -')
    logging.info('API Program Start Datetime:     [  '+str(time.strftime('%y-%m-%d %H:%M:%S'))+'  ]')
    #------------------[    Log Part     ]-------------------------------------------------[-]
...
#---[Oracle]--------------------------------------------------------[Check Part]------------------------------------------[+]
    ...
    def getGlobStore(self):
        #  取当前店铺的全局店铺号码
        self.cur.execute(
            "select s.glob_store_code from cms.default_store df,cms.store s where df.sbs_no=1 and df.default_doc_type=0 and df.store_no=s.store_no and s.sbs_no=1")
        ...
        if getstr!=None:
           ...
           logging.info(self.getCurrDatetime()+'  |Error|-------->     Glob_store not found')#   Append Log
       ...
       # -------Loop List -----------------------(+)
           ...
           for docList in JsonStr['content']['result']['list']:
               ...
               if str_checkinvc == None:
                   ...
                   logging.info(self.getCurrDatetime()+'  |Note|-------->   Document already exist '+str(str_checkinvc))#   Append Log

               str_checkcust = self.customer_check(str(docList['buyerId']))
               if str_checkcust ==  None:
                   print('Customer Not Exist')
                   documentError = 1
                   logging.info(self.getCurrDatetime()+'  |Error|-------->   Customer Not Exist '+str(docList['buyerId']))#   Append Log
               else:
                   print('Cust is Found ')
                 ...
                 # ------ Check Item ---------------------------------[+]
                    ...
                    if str_checkitem != None:
                       if self.test_type=='test':                 #----test-------
                          print('Item Sid:'+str(str_checkitem[1]))#----test-------
                          print('List[3]:'+str(str_checkitem[3])) #----test-------
                    else:
                       print('  |Error|-------->[Item Not Found]  Document No:'+docList['dealCode']+'   Barcode:'+docDetail['barCode'])
                       documentError = documentError + 1
                       logging.info(self.getCurrDatetime()+'  |Error|-------->[Item Not Found]  Document No:'+docList['dealCode']+'   Barcode:'+docDetail['barCode'])#   Append Log
                  # ------- Check if document not error then insert to created list -------[+]
                 if documentError == 0:
                    self.CreateDOCList.append(docList['dealCode'])
                    if self.test_type=='test':
                       print(docList['dealCode']+' will Created')#----test-------
                  # -----------------------------------------------------------------------[-]
...
#---------[     Create XML part       ]----------------------------------------------------------------------------------------------------------------------------------------[+]
         if str(JsonStr['success'])=='True' and str(JsonStr['content']['result']['pageCond']['count'])>'1':
            ...
            logging.info(self.getCurrDatetime()+'  |Note|-------->[Create  XML Document]')#   Append Log 
