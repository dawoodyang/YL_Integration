...
#[Class Setup info]---------------------------------------------------------------------------------------------------[-]     
    ...
    def __init__(self,p_platform_num,p_startDateTime,p_endDateTime):
         ...
         self.t_startDateStamp = str(int(time.mktime(time.strptime(p_startDateTime, "%Y-%m-%d %H:%M:%S") ) ) )
         self.t_EndDateStamp   = str(int(time.mktime(time.strptime(p_endDateTime, "%Y-%m-%d %H:%M:%S") ) ) ) 
    ...
    def itemCheck(self,p_itemcode):
        ...
        self.cur.execute("select item_sid,local_upc,description2,text3 from cms.INVN_SBS where ALU="+p_itemcode)
    ...
    def invcCheck(self,p_invcno):
        ...
        self.cur.execute("select invc_sid,tracking_no from cms.invoice where tracking_no='"+p_invcno+"'")
    ...
    def gettoken(self):
         ...
         self.t_token = json.loads(get_respons.text).get('content').get('result')
         print('---------   Get Token   ---------')
    ...
    def getorder(self): 
         ...
         paramJSON                       = {'_platform_num':self.t_platform_num,'pageIndex':1,'pageSize':10,'createTimeFrom':self.t_startDate,'createTimeTo':self.t_endDate,'deptCode':8888}
         ...
         str_sign = 'key='+self.t_token+'&_platform_num='+self.t_platform_num+'&timeStamp='+self.t_timeStamp        
         md5 = hashlib.md5()  
         ...
         #try:
         if (1==1):
           get_respons = requests.post(url=self.url_getorder,headers = headers,data=json.dumps(paramJSON),verify = False )  
           #print(get_respons.text)
           #-------Get JSON Str---------------------
           JsonStr = json.loads(get_respons.text)          
           print('success:'+str(JsonStr['success']))
           print('PageCount:'+str(JsonStr['content']['result']['pageCond']['count']))
           #-------Get JSON Str---------------------
#[JSON-Check Document / Customer / Items]-----------------------------------------------------------------------------------------------------------------------[+]           
       #-------Loop List -----------------------(+)
           ...
           for docList in JsonStr['content']['result']['list']:
               ...
               print('------------------ Header ---------------------')
               #print(docList)
               print('[Header]-------->  DocumentNo:'+docList['dealCode']+'   Date:'+str(docList['createTime'])+'  SaleFlag:'+str(docList['saleFlag'])+'  RecvName:'+docList['recvName']+'  BuyerId:'+str(docList['buyerId'])+' TotalPayment:'+str(docList['dealTotalFee']))
               ...
               if str_checkinvc == None:
                 for docDetail in docList['details']:
                    ...
                    print('[Lines]--------->   DocumentNo:'+docList['dealCode']+'   ITEM NO:'+docDetail['goodsName']+'  Name:'+docDetail['barCode']+'   Quantity:'+str(docDetail['amount'])+'  Price:' +str(docDetail['tradePrice']))
                 #------ Check Item ---------------------------------[+]
...
API = webservice('101397','2020-08-07 00:00:00','2020-08-07 23:59:59')
API.gettoken()
API.getorder()
